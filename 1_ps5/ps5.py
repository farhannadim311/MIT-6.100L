...
def filter(pixels_list, color):
    ...
    for i in pixels_list:
        tmp = tuple(matrix_multiply(matrix, i))
        result = (int(tmp[0]), int(tmp[1]), int(tmp[2]))
        converted_image = converted_image + (result ,) 
    return list(converted_image)
...
def reveal_bw_image(filename):
    ...
    size = im.size
    pix = img_to_pix(filename)
    lsb = []
    for i in pix:
        lsb.append(extract_end_bits(1, i))   
    img = pix_to_img(lsb, size, "L") 
    img.putdata(lsb, scale = 255)
    img_save = img.save("bw.png")
    return img
...
def reveal_color_image(filename):
    ...
    for i in pix:
        lsb.append(extract_end_bits(3, i)) 
    img = pix_to_img(lsb, size, "RGB") 
    pix_new = img.getdata()
    for i in pix_new:                                   # I could not get the putdata offset to work hence I did it manually 
        new.append((24 * i[0], 24 * i[1], 24 * i[2]))    
    img = pix_to_img(new, size, "RGB")
    img_save = img.save("rgb.png")
    return img
...
def main():
    ...
    draw_kerb("bw.png", "rat")
    draw_kerb("rgb.png", "rat")    
