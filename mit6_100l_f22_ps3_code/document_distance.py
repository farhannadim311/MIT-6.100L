...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for word in unique_words:
        if ((word in freq_dict1) and word in freq_dict2):
            frequency_diff[word] = abs(freq_dict1[word] - freq_dict2[word])
            frequency_total[word] = freq_dict1[word] + freq_dict2[word]
        elif (word in freq_dict1):
            frequency_diff[word] = freq_dict1[word]
            frequency_total[word] = freq_dict1[word]
        else:
            frequency_diff[word] = freq_dict2[word]
            frequency_total[word] = freq_dict2[word]
    for word in frequency_diff:
        num = num + frequency_diff[word]
    for word in frequency_total:
        denom = denom + frequency_total[word]
    score = 1 - (num/denom)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    unique_dict ={}
    sorted_words = []
    for words in freq_dict1:  ## Making a unique word dictionary
        if words not in unique_dict:
            unique_dict[words] = freq_dict1[words]
        else:
            unique_dict[words] = unique_dict[words] + freq_dict1[words]
    for words in freq_dict2:
        if words not in unique_dict:
            unique_dict[words] = freq_dict2[words]
        else:
            unique_dict[words] = unique_dict[words] + freq_dict2[words]
    keys = unique_dict.keys() ## sorting it alphabetically
    values = unique_dict.values() ## getting the highest value
    values = list(values)
    for word in keys:
        if (unique_dict[word] == values[len(values) -1]):
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    text = text_to_list(file_contents)
    total_words = len(text)
    tf = {}
    word_frequencies = get_frequencies(text)
    for word in word_frequencies:
        tf[word] = word_frequencies[word] / total_words
    return tf
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    doc_words  = {}
    idf = {}
    for i in file_paths:                     ##Nested loop counts the number of times words occur in document as it iterates through each file
        file_contents = load_file(i)
        text = text_to_list(file_contents)
        words = get_frequencies(text)
    ...
    for elem in doc_words:
        idf[elem] = math.log10(number_of_files / doc_words[elem])
    return idf
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    sorted_tuple = sorted(tfidf.items(), key = lambda kv: (kv[1], kv[0]))
    return sorted_tuple
